...
class CombinedControlNode(Node):
   def __init__(self):
       ...
       self.get_logger().info("Combined Control Node Initialized")
   ...
   def control_steering(self, msg):
       ...
       self.get_logger().info(f'Received lane offset: {self.lane_offset:.2f}')
   ...
   def publish_control_command(self):
       ...
       steering_gain = 0.5  # You can tune this value to adjust steering sensitivity
       drive_msg.steering_angle = -self.lane_offset * steering_gain
       ...
       self.get_logger().info(f'Publishing control: speed={drive_msg.speed}, steering_angle={drive_msg.steering_angle:.2f}')
